...
export async function vistaUsuarios({ir, refrescar}) {
  ...
  function opcionesResponsable(u) {
    return [['', 'Nadie']].concat(api.estado.usuarios
      .filter(x => (!u || String(x.id) !== String(u.id)) && ['comercial', 'admin', 'superadmin'].includes(x.rol))
      .map(x => [x.id, x.nombre]));
  }
  ...
  function campos(u) {
    return [
      ...
      {separador: 'Lo que cobra por vender'},
      {id: 'comision_fv', et: 'Comisión fotovoltaica (€)', tipo: 'euro',
       ayuda: 'A cero para quien no cierra ventas'},
      {id: 'comision_aero', et: 'Comisión aerotermia (€)', tipo: 'euro'},
      {id: 'comision_fv_ajustada', et: 'FV con precio ajustado (€)', tipo: 'euro',
       ayuda: 'La que se aplica cuando la instalación va marcada como precio ajustado'},
      {id: 'comision_aero_ajustada', et: 'Aerotermia con precio ajustado (€)', tipo: 'euro'},

      {separador: 'Lo que cobra por captar'},
      {id: 'comision_captacion_fv', et: 'Captación de fotovoltaica (€)', tipo: 'euro',
       ayuda: 'Se paga a quien abrió la puerta, la cierre quien la cierre'},
      {id: 'comision_captacion_aero', et: 'Captación de aerotermia (€)', tipo: 'euro'},

      {separador: 'Lo que cobra por su equipo'},
      {id: 'responsable_id', et: 'Su responsable', tipo: 'select', opciones: opcionesResponsable(u),
       ayuda: 'Quien cobra comisión de equipo por lo que cierra esta persona'},
      {id: 'comision_equipo_fv', et: 'FV que venda su equipo (€)', tipo: 'euro',
       ayuda: 'Lo que cobra esta persona por cada venta de quien tiene debajo'},
      {id: 'comision_equipo_aero', et: 'Aerotermia que venda su equipo (€)', tipo: 'euro'},
  ...
  function editor(u) {
    ventanaFormulario({
      ...
      valores: u || {rol: 'comercial', activo: 'si', fecha_alta: hoyISO(), irpf_pct: 15, ss_pct: 6.35,
        ...
        comision_fv_ajustada: 200, comision_aero_ajustada: 200,
        comision_captacion_fv: 400, comision_captacion_aero: 400,
        comision_equipo_fv: 0, comision_equipo_aero: 0, responsable_id: '', objetivo_mes: 6},
      alGuardar: async d => {
        if (u) d.id = u.id;
...
export async function vistaAjustes({refrescar}) {
  ...
  const grupos = [
    ...
    ['Comisiones y objetivos', [
      ...
      ['comision_captacion_fv', 'Captación de FV vendida (€)'],
      ['comision_captacion_aero', 'Captación de aerotermia vendida (€)'],
      ['comision_equipo_fv', 'FV que venda el equipo, al responsable (€)'],
      ['comision_equipo_aero', 'Aerotermia que venda el equipo, al responsable (€)'],
    ...
    ['El correo de los viernes', [
      ['resumen_semanal', '¿Se manda el resumen semanal? (si/no)'],
      ['resumen_a', 'Mandarlo todo a un solo correo (vacío = cada uno el suyo)'],
      ['copia_resumen', 'Copia del envío para quien lo controle']]],
    ['Herramientas', [
      ...
      tarjeta('Tus datos', h('div',
        ...
        yo.salario_bruto !== undefined ? h('div',
          ...
          h('.dato', h('.et', 'Comisión FV / aerotermia'),
            h('.v', eur(yo.comision_fv) + ' / ' + eur(yo.comision_aero))),
          num(yo.comision_fv_ajustada) ? h('.dato', h('.et', 'Con precio ajustado'),
            h('.v', eur(yo.comision_fv_ajustada) + ' / ' + eur(yo.comision_aero_ajustada))) : null,
          num(yo.comision_captacion_fv) ? h('.dato', h('.et', 'Por captar'),
            h('.v', eur(yo.comision_captacion_fv) + ' / ' + eur(yo.comision_captacion_aero))) : null,
          num(yo.comision_equipo_fv) ? h('.dato', h('.et', 'Por venta de su equipo'),
            h('.v', eur(yo.comision_equipo_fv) + ' / ' + eur(yo.comision_equipo_aero))) : null) : null,
