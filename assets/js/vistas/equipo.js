  ...
  return h('div',
    ...
    r.comisiones.detalle.length ? tarjeta('Detalle de comisiones',
      tabla([
        ...
        {clave: 'concepto', et: 'Por qué', valor: c => txt(c.concepto) || 'venta',
         pinta: c => h('span', capital(txt(c.concepto) || 'venta') +
           (txt(c.de) ? ' · ' + c.de : ''))},
        {clave: 'tipo', et: 'Qué', valor: c => api.etiquetaCatalogo('tiposOperacion', c.tipo) || c.tipo},
        {clave: 'importe', et: 'Comisión', num: true, pinta: c => h('b', eur(c.importe))}
      ], r.comisiones.detalle, {pie: [{texto: 'Total', colspan: 4}, {texto: eur(r.comisiones.total), num: true}]}),
